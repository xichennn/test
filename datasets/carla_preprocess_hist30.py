...
class CarlaDataset(Dataset):
    ...
    def processed_dir(self) -> str:
        return os.path.join(self.root, 'scene_mining_hivt/processed30', self._split)
    ...
    def process_carla(self,
        ...
        # and consider actors only in sensing range
        # consider only 3s historical timesteps
        timestamps = list(np.sort(df['frame'].unique()))
        #historical time=30
        historical_timestamps = timestamps[20: 50]
        historical_df = df[df['frame'].isin(historical_timestamps)]
        historical_sdf = historical_df[historical_df["in_av_range"]==True]
        actor_ids = list(historical_sdf['vid'].unique())
        df = df[df['vid'].isin(actor_ids)]
        df = df[df['frame'].isin(timestamps[20:])]
        ...
        origin = torch.tensor([cav_df[29]['position_x'], cav_df[29]['position_y']], dtype=torch.float)
        cav_heading_vector = origin - torch.tensor([cav_df[28]['position_x'], cav_df[28]['position_y']], dtype=torch.float)
        theta = torch.atan2(cav_heading_vector[1], cav_heading_vector[0])
        rotate_mat = torch.tensor([[torch.cos(theta), -torch.sin(theta)],
                                    [torch.sin(theta), torch.cos(theta)]])

        # initialization
        x = torch.zeros(num_nodes, 80, 2, dtype=torch.float)
        edge_index = torch.LongTensor(list(permutations(range(num_nodes), 2))).t().contiguous()
        padding_mask = torch.ones(num_nodes, 80, dtype=torch.bool)
        bos_mask = torch.zeros(num_nodes, 30, dtype=torch.bool)
        rotate_angles = torch.zeros(num_nodes, dtype=torch.float)

        for actor_id, actor_df in df.groupby('vid'):
            node_idx = actor_ids.index(actor_id)
            node_steps = [timestamps[20:].index(timestamp) for timestamp in actor_df['frame']]
            padding_mask[node_idx, node_steps] = False
            if padding_mask[node_idx, 29]:  # make no predictions for actors that are unseen at current timestep
                padding_mask[node_idx, 30:] = True
            xy = torch.from_numpy(np.stack([actor_df['position_x'].values, actor_df['position_y'].values], axis=-1)).float() #[80,2]
